...
main (int argc, char *argv[])
{
  ...
  // std::vector<Ptr<MmWaveEnbNetDevice>> enbNetDeviceContainer;
  // std::vector<Ptr<MmWaveUeNetDevice>> ueNetDeviceContainer;

  // for (int i = 0; i < numberOfEnbs; i++)
  //   {
  //     enbNetDeviceContainer.push_back (enbDevs.Get (i)->GetObject<MmWaveEnbNetDevice> ());
  //   }

  // for (int i = 0; i < numberOfUes; i++)
  //   {
  //     ueNetDeviceContainer.push_back (ueDevs.Get (i)->GetObject<MmWaveUeNetDevice> ());
  //   }

  std::vector<Ptr<LteEnbRrc>> enbRrcs;
  for (int i = 0; i < numberOfEnbs; i++)
    {
      enbRrcs.push_back (enbDevs.Get (i)->GetObject<mmwave::MmWaveEnbNetDevice> ()->GetRrc ());
    }
  ...
  // std::cout << enbDevs.Get (0)->GetObject<mmwave::MmWaveEnbNetDevice> ()->GetCellId ()
  //           << "++++++++++++++++++++++++++++++++++" << std::endl;
  ...
  Simulator::Schedule (Seconds (0.3), &PerformHandover, enbDevs.Get (1), ueDevs.Get (0),
                       enbDevs.Get (0));
