...
void
reportParams (NetDeviceContainer ueDevs, uint16_t indx, Ptr<sdnClient> ch)
{
...
main (int argc, char *argv[])
{
  ...
  for (uint16_t i = 0; i < numberOfEnbs - 1; i++)
    {
      Vector enbPosition (distance * (i + 1), distance, 0);
      enbPositionAlloc->Add (enbPosition);
    }

  Vector enbPosition (750, 750, 0);
  enbPositionAlloc->Add (enbPosition);
  ...
  pos.Set ("X", StringValue ("ns3::UniformRandomVariable[Min=250.0|Max=1000.0]"));
  pos.Set ("Y", StringValue ("ns3::UniformRandomVariable[Min=250.0|Max=1000.0]"));

  std::ostringstream speedConstantRandomVariableStream;
  speedConstantRandomVariableStream << "ns3::ConstantRandomVariable[Constant=" << 300 << "]";
  ...
  for (int i = 0; i < numberOfUes; i++)
    {
      Simulator::Schedule (Seconds (0.1 + a), &reportParams, ueDevs, i, sdnClients[i]);
      a = a + a;
    }
