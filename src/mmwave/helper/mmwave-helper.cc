  ...
  static TypeId tid =
      TypeId ("ns3::MmWaveHelper")
          ...
          .AddAttribute ("BasicImsi", "The next value will be the first  imsi", UintegerValue (0),
                         ...
                         MakeUintegerChecker<uint16_t> ())
          .AddAttribute ("NumUePanels", "Number of panels for the UE", UintegerValue (1),
                         MakeUintegerAccessor (&MmWaveHelper::m_noUePanels),
                         MakeUintegerChecker<uint16_t> ())
          .AddAttribute ("NumEnbPanels", "Number of panels for the eNB", UintegerValue (1),
  ...
  for (std::map<uint8_t, MmWaveComponentCarrier>::iterator it =
       ...
       it != m_componentCarrierPhyParams.end (); ++it)
    {
      ...
      if (!m_pathlossModelType.empty ())
        {
          ...
          if (m_pathlossModelType == "ns3::BuildingsObstaclePropagationLossModel")
            {
              Ptr<MmWaveLosTracker> losTracker =
                  CreateObject<MmWaveLosTracker> (); // create and initialize m_losTracker
              Ptr<BuildingsObstaclePropagationLossModel> building =
                  pathlossModel->GetObject<BuildingsObstaclePropagationLossModel> ();
              building->SetConfigurationParameters (phyMacCommon);
              //building->SetBeamforming (m_beamforming); //dove viene creato m_beamforming??
              building->SetLosTracker (
                  losTracker); // use m_losTracker in BuildingsObstaclePropagationLossModel

              m_losTracker[it->first] = losTracker;
            }
          else if (m_pathlossModelType == "ns3::MmWavePropagationLossModel")
            {
              pathlossModel->GetObject<MmWavePropagationLossModel> ()->SetConfigurationParameters (
                  phyMacCommon);
            }
          else if (m_pathlossModelType == "ns3::MmWave3gppPropagationLossModel")
            {
              pathlossModel->GetObject<MmWave3gppPropagationLossModel> ()
                  ->SetConfigurationParameters (phyMacCommon);
            }
          else if (m_pathlossModelType == "ns3::MmWave3gppBuildingsPropagationLossModel")
            {
              pathlossModel->GetObject<MmWave3gppBuildingsPropagationLossModel> ()
                  ->SetConfigurationParameters (phyMacCommon);
            }
      ...
      if (m_channelModelType == "ns3::MmWaveBeamforming")
        {
          Ptr<MmWaveBeamforming> beamforming =
              CreateObject<MmWaveBeamforming> (m_noTxAntenna, m_noRxAntenna);
      ...
      else if (m_channelModelType == "ns3::MmWave3gppChannel")
        {
          ...
          if (m_pathlossModelType == "ns3::MmWave3gppBuildingsPropagationLossModel" ||
              m_pathlossModelType == "ns3::MmWave3gppPropagationLossModel")
            {
              Ptr<PropagationLossModel> pl =
            ...
            {
              NS_FATAL_ERROR (
                  "The 3GPP channel and propagation loss should be enabled at the same time");
            }
...
MmWaveHelper::InstallSingleMcUeDevice (Ptr<Node> n)
{
  ...
  for (std::map<uint8_t, Ptr<MmWaveComponentCarrierUe>>::iterator it = mmWaveUeCcMap.begin ();
       it != mmWaveUeCcMap.end (); ++it)
    {
      ...
      Ptr<MmWaveHarqPhy> harq =
          Create<MmWaveHarqPhy> (it->second->GetConfigurationParameters ()->GetNumHarqProcess ());
...
MmWaveHelper::InstallSingleUeDevice (Ptr<Node> n)
{
  ...
  for (std::map<uint8_t, Ptr<MmWaveComponentCarrierUe>>::iterator it = ueCcMap.begin ();
       it != ueCcMap.end (); ++it)
    {
      Ptr<MmWaveSpectrumPhy> ulPhy = CreateObject<MmWaveSpectrumPhy> ();
      Ptr<MmWaveSpectrumPhy> dlPhy = CreateObject<MmWaveSpectrumPhy> ();
  ...
  for (std::map<uint8_t, Ptr<MmWaveComponentCarrierUe>>::iterator it = ueCcMap.begin ();
       it != ueCcMap.end (); ++it)
    {
      ...
      ccPhy->GetDlSpectrumPhy ()->SetPhyRxCtrlEndOkCallback (
          MakeCallback (&MmWaveUePhy::ReceiveControlMessageList, ccPhy));
      // ccPhy->GetDlSpectrumPhy ()->SetPhyRxPssCallback (MakeCallback (&MmWaveUePhy::ReceivePss, ccPhy));
      // ccPhy->GetDlSpectrumPhy ()->SetLtePhyDlHarqFeedbackCallback (MakeCallback (&LteUePhy::ReceiveLteDlHarqFeedback, ccPhy)); this is done before
    }
...
MmWaveHelper::InstallSingleEnbDevice (Ptr<Node> n)
{
  ...
  for (std::map<uint8_t, Ptr<MmWaveComponentCarrierEnb>>::iterator it = ccMap.begin ();
       it != ccMap.end (); ++it)
    {
          ...
          if (splm)
            {
              ...
              if (m_losTracker.find (it->first) != m_losTracker.end ())
                {
                  phy->AddLosTracker (m_losTracker.at (
                      it->first)); // use m_losTracker in phy (and in particular in enbPhy)
                }
            }
        }
      else
  ...
  for (std::map<uint8_t, Ptr<MmWaveComponentCarrierEnb>>::iterator it = ccMap.begin ();
       it != ccMap.end (); ++it)
    {
      ...
      bandwidthMap[it->first] = phyMacConfig->GetNumRb () * phyMacConfig->GetChunkWidth () *
                                phyMacConfig->GetNumChunkPerRb ();
  ...
  for (std::map<uint8_t, Ptr<MmWaveComponentCarrierEnb>>::iterator it = ccMap.begin ();
       it != ccMap.end (); ++it)
    {
      ...
      rrc->SetLteEnbCphySapProvider (it->second->GetPhy ()->GetMmWaveEnbCphySapProvider (),
                                     it->first);
      ...
      //Scheduler SAP
      it->second->GetMac ()->SetMmWaveMacSchedSapProvider (
          it->second->GetMacScheduler ()->GetMacSchedSapProvider ());
      it->second->GetMac ()->SetMmWaveMacCschedSapProvider (
          it->second->GetMacScheduler ()->GetMacCschedSapProvider ());

      it->second->GetMacScheduler ()->SetMacSchedSapUser (
          it->second->GetMac ()->GetMmWaveMacSchedSapUser ());
      it->second->GetMacScheduler ()->SetMacCschedSapUser (
          it->second->GetMac ()->GetMmWaveMacCschedSapUser ());
      // Scheduler SAP END
...
*/
  for (std::map<uint8_t, Ptr<MmWaveComponentCarrierEnb>>::iterator it = ccMap.begin ();
       it != ccMap.end (); ++it)
    {
      ...
      if (!m_pathlossModelType.empty ())
        {
          bool freqOk = m_pathlossModel.at (it->first)->SetAttributeFailSafe ("Frequency",
                                                                              DoubleValue (freq));
          if (!freqOk)
            {
  ...
  for (std::map<uint8_t, Ptr<MmWaveComponentCarrierEnb>>::iterator it = ccMap.begin ();
       it != ccMap.end (); ++it)
    {
      ...
      m_channel.at (it->first)->AddRx (
          it->second->GetPhy ()->GetDlSpectrumPhy ()); //TODO check if Dl and Ul are the same
    }
          ...
          for (NetDeviceContainer::Iterator enbIter = enbDevices.Begin ();
               enbIter != enbDevices.End (); ++enbIter)
            {
              Ptr<AntennaArrayModel> ueAntennaArray =
                  DynamicCast<AntennaArrayModel> ((DynamicCast<MmWaveUeNetDevice> (*i))
                                                      ->GetPhy (it->first)
                                                      ->GetDlSpectrumPhy ()
                                                      ->GetRxAntenna ());
...
MmWaveHelper::AttachToClosestEnb (Ptr<NetDevice> ueDevice, NetDeviceContainer enbDevices)
{
  ...
  for (NetDeviceContainer::Iterator i = enbDevices.Begin (); i != enbDevices.End (); ++i)
    {
      ...
      for (std::map<uint8_t, Ptr<MmWaveComponentCarrierEnb>>::iterator itEnb = enbCcMap.begin ();
           itEnb != enbCcMap.end (); ++itEnb)
        {
          uint16_t mmWaveCellId = itEnb->second->GetCellId ();
          Ptr<MmWavePhyMacCommon> configParams =
              itEnb->second->GetPhy ()->GetConfigurationParameters ();
          ...
          for (std::map<uint8_t, Ptr<MmWaveComponentCarrierUe>>::iterator itUe = ueCcMap.begin ();
               itUe != ueCcMap.end (); ++itUe)
            {
  ...
  Ptr<MmWavePhyMacCommon> configParams =
      closestEnbDevice->GetObject<MmWaveEnbNetDevice> ()->GetPhy ()->GetConfigurationParameters ();

  closestEnbDevice->GetObject<MmWaveEnbNetDevice> ()->GetPhy ()->AddUePhy (
  ...
  closestEnbDevice->GetObject<MmWaveEnbNetDevice> ()->GetMac ()->AssociateUeMAC (
  ...
  ueDevice->GetObject<MmWaveUeNetDevice> ()->SetTargetEnb (
      closestEnbDevice->GetObject<MmWaveEnbNetDevice> ());
  //}
}
                                    ...
                                    NetDeviceContainer lteEnbDevices)
{
  ...
  NS_ASSERT (lteClosestEnbDevice->GetObject<LteEnbNetDevice> () !=
             0); // stop if it is not an LTE eNB
  ...
  for (NetDeviceContainer::Iterator i = mmWaveEnbDevices.Begin (); i != mmWaveEnbDevices.End ();
       ++i)
    {
      ...
      for (std::map<uint8_t, Ptr<MmWaveComponentCarrierEnb>>::iterator itEnb =
               mmWaveEnbCcMap.begin ();
           itEnb != mmWaveEnbCcMap.end (); ++itEnb)
        {
          uint16_t mmWaveCellId = itEnb->second->GetCellId ();
          Ptr<MmWavePhyMacCommon> configParams =
              itEnb->second->GetPhy ()->GetConfigurationParameters ();
...
uint8_t
MmWaveHelper::ActivateDedicatedEpsBearer (Ptr<NetDevice> ueDevice, EpsBearer bearer,
                                          Ptr<EpcTft> tft)
{
  NS_LOG_FUNCTION (this);
...
MmWaveHelper::EnableDlPhyTrace (void)
{
  ...
  Config::Connect (
      "/NodeList/*/DeviceList/*/ComponentCarrierMapUe/*/MmWaveUePhy/DlSpectrumPhy/RxPacketTraceUe",
...
{
  NS_LOG_FUNCTION_NOARGS ();
  Config::Connect ("/NodeList/*/DeviceList/*/ComponentCarrierMap/*/MmWaveEnbPhy/DlSpectrumPhy/"
...
{
  NS_LOG_FUNCTION_NOARGS ();
  Config::Connect ("/NodeList/*/DeviceList/*/ComponentCarrierMapUe/*/MmWaveUePhy/DlSpectrumPhy/"
